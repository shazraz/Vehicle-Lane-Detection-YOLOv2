...
class ImageProcessor: 
    ...
    def lane_detection(self, img):
        ...
        binary = lane_utils.create_binary(img, self.img_points, self.obj_points, self.mtx, self.dist, 
                                          self.L_thresh, self.B_thresh)
        ...
        if (self.left_line.centers.shape[0] > 2 ) & (self.right_line.centers.shape[0] > 2):
            ...
            self.left_line.coeff = lane_utils.get_coeff(self.log, self.left_line.centers)
            self.log.write('{} Getting Right Line Co-efficients\n'.format(datetime.datetime.now()))
            self.right_line.coeff = lane_utils.get_coeff(self.log, self.right_line.centers)
