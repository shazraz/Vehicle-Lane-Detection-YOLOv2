...
def _main(args):
    ...
    for section in cfg_parser.sections():
        ...
        if section.startswith('convolutional'):
            ...
            # Create Conv2D layer
            conv_layer = (Conv2D(
                filters, size, size,#remove brackets around size
                subsample=(stride, stride),
                W_regularizer=l2(weight_decay),#change kernel regularizer to W
                bias=not batch_normalize,#change bias kwarg
                weights=conv_weights,
                activation=act_fn,
                border_mode=padding))(prev_layer) #change padding to border mode
            ...
            all_layers.append(
                MaxPooling2D(
                    border_mode='same',#change to border mode
                    pool_size=(size, size),
            ...
            if len(layers) > 1:
                ...
                concatenate_layer = merge(layers, mode='concat')#concatenate(layers)#changed call back to Keras 1.2.2
    ...
    model = Model(input=all_layers[0], output=all_layers[-1])#change inputs/outputs to input/output
    print(model.summary())
